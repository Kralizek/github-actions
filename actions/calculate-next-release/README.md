...
An explicit `version` can be supplied to bypass derived version calculation. Stable overrides must use `major.minor.patch`. Prerelease overrides must match the selected channel and use `major.minor.patch-channel.number`. If the resulting tag already exists on `HEAD`, the action reuses it; if it points to a different commit, the action fails.

Rerunning the same release operation on a commit that already has exactly one matching release tag reuses that tag. This applies to both stable and prerelease releases. If multiple matching stable tags point to `HEAD`, or multiple matching prerelease tags for the same base version and channel point to `HEAD`, the action fails rather than choosing one arbitrarily.
